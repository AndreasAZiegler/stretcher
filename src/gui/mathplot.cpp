...
void mpFXYVector::AddXData(double x_value){
  m_temp_xs.push_back(x_value);
}

void mpFXYVector::AddYData(double y_value){
  m_temp_ys.push_back(y_value);
}

void mpFXYVector::PrepareXYData(void){
  if(m_temp_ys.size() > m_temp_xs.size()){
    if(false == m_temp_xs.empty()){
      while(m_temp_ys.size() > m_temp_xs.size()){
        m_temp_xs.push_back(m_temp_xs.back()); 
      }
    } else{
      m_temp_ys.resize(m_temp_xs.size());
    }
  } else{
    m_temp_xs.resize(m_temp_ys.size());
  }

  for(int i = 0; i < m_temp_xs.size(); ++i){
    m_xs.push_back(m_temp_xs[i]); 
    m_ys.push_back(m_temp_ys[i]); 
  }
  m_temp_xs.clear();
  m_temp_ys.clear();
